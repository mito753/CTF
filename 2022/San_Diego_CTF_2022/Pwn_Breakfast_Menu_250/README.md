...
Dockerfileの抜粋でnsjailで実行していることが原因かも。
```
...
`libc`アドレスをリークできれば、`tcache`に`__free_hook`を容易に書き込める。更に`__free_hook`に`system`関数のアドレスを書き込めるので、`/bin/sh`を事前に書き込んだ`chunk`をfreeすることで`/bin/sh`を起動できる。
